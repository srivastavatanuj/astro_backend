...
class ChatRequestController extends Controller
{
    public function addChatRequest(Request $req)
    {
        try {
            ...
            if ($userDeviceDetail && count($userDeviceDetail) > 0) {
                ...
                //     [
                //         'title' => 'Hey '.$userDeviceDetail[0]->name.', you received a chat request from ' . $user[0]->name,
                //         'body' => ['description' => 'Hey '.$userDeviceDetail[0]->name.', you received a chat request from ' . $user[0]->name, 'notificationType' => 8, 'icon' => 'public/notification-icon/chat.png','sound' =>'ringtone'],
				...
				$chatrequestid=ChatRequest::latest()->first();
                $notification = array(
                    ...
                    'notification_type' => 8,
					'chatRequestId' => $chatrequestid->id
                );
    ...
    public function getChatRequest(Request $req)
    {
        try {
            ...
            $chatRequest = DB::table('chatrequest')
                ...
                ->selectRaw('users.*,chatrequest.id as chatId,chatrequest.senderId,user_device_details.fcmToken, chatrequest.chat_duration,chatrequest.created_at as chatcreatedat,chatrequest.astrologerId,chatrequest.userId');
                    ...
                    $response = FCMService::send(
                        ...
                        [
                            ...
                            'body' => [
                                ...
                                // 'description' => 'Congrats, your chat request accepted by ' . $astrologer[0]->name,
        ...
        try {
            ...
            if ($userDeviceDetail && count($userDeviceDetail) > 0) {
                $response = FCMService::send(
                    $userDeviceDetail,
                    [
                        'title' => 'Hey '.$astrologer[0]->name.', we received a call request.',
                        'body' => [
                            "astrologerId" => $chatRequest->astrologerId,
                            "astrologerName" => $astrologer[0]->name,
                            "notificationType" => 6,
                            ...
                            "receiverId" => $chatRequest->receiverId,
                            "senderId" => $chatRequest->senderId,
                            'description' => 'Hey '.$astrologer[0]->name.', we received a call request.',
                            'icon' => 'public/notification-icon/chat.png',
                        ],
                    ]
                );
        ...
        try {
            ...
            if ($userDeviceDetail && count($userDeviceDetail) > 0) {
                FCMService::send(
                    $userDeviceDetail,
                    [
                        'title' => 'Hey '.$astrologer[0]->name.', we received a chat request',
                        'body' => [
                            ...
                            "chatId" => $req->chatId,
                            'description' => 'Hey '.$astrologer[0]->name.', we received a chat request',
                            'icon' => 'public/notification-icon/chat.png',
                        ],
                    ]
                );
    ...
    public function endChatRequest(Request $req)
    {
        try {
            ...
            // Get the difference in seconds between now and the 'updated_at' field
        ...
        try {
            ...
            if (!($sId && count($sId) > 0)) {
                ...
                if($req->callType=='Video'){
                    $charge[0]->charge=$charge[0]->videoCallRate;
                }else{
